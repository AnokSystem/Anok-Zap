      ...
      <CardContent>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
          {instances.map((instance) => (
            <div key={instance.id} className="flex items-center space-x-2 p-3 bg-gray-50 border border-gray-200 rounded-lg data-[theme=dark]:bg-gray-700 data-[theme=dark]:border-gray-600">
              <Checkbox
                id={instance.id}
                checked={selectedInstances.includes(instance.id)}
                onCheckedChange={() => onInstanceToggle(instance.id)}
              />
              <Label htmlFor={instance.id} className="text-gray-700 flex-1 cursor-pointer data-[theme=dark]:text-gray-300">
                {instance.name}
        ...
        {selectedInstances.length > 0 && (
          <div className="mt-3 p-2 bg-purple-50 border border-purple-200 rounded data-[theme=dark]:bg-purple-900/20 data-[theme=dark]:border-purple-700">
